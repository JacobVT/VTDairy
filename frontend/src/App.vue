<template>
  <v-app v-on:scroll="handleScroll" id="dairy">
    <v-app-bar
      app
      color="primary"
      elevate-on-scroll
    >
      <!-- Show home icon and label when user is NOT on main landing page -->
       <!-- <v-btn icon dark>
        <v-icon>mdi-home</v-icon>
      </v-btn> -->
      <v-toolbar-title class="f-martel white--text">Van Tol Dairy</v-toolbar-title>

      <v-spacer></v-spacer>

      <v-tooltip bottom>
        <template v-slot:activator="{ on, attrs }">
          <v-btn
            color="white"
            text
            v-bind="attrs"
            v-on="on"
          >Recipes</v-btn>
        </template>
        <span>Still in development!</span>
      </v-tooltip>

      <v-tooltip bottom>
        <template v-slot:activator="{ on, attrs }">
          <v-btn
            color="white"
            text
            v-bind="attrs"
            v-on="on"
          >Games</v-btn>
        </template>
        <span>Still in development!</span>
      </v-tooltip>

      <v-tooltip bottom>
        <template v-slot:activator="{ on, attrs }">
          <v-btn
            color="white"
            text
            v-bind="attrs"
            v-on="on"
          >Contact us</v-btn>
        </template>
        <span>Still in development!</span>
      </v-tooltip>
    </v-app-bar>

    <v-main>
      <v-container>
        <v-row justify="center" align="center" class="pb-5" >
          <v-col class="text-center" cols="12">
            <h5 class="f-dancing h5-size" > Welcome to the</h5>
            <h6 class="f-martel h6-size stroke" style=" color: skyblue"> Van Tol Dairy </h6>
            <!-- <h2 class="text-h2">
              <v-icon dark left>mdi-star</v-icon>
              website
              <v-icon dark right>mdi-star</v-icon>
            </h2> -->
          </v-col>
        </v-row>
        <v-row 
        justify="center" align="center"
        v-observe-visibility="visibilityChanged">
          <v-fade-transition>
            <v-slide-group
              v-if="cardVisible"
              v-model="slide"
            >
             <v-slide-item
                v-slot:default="{ active, toggle }"
              >
                <v-card
                  class="ma-4"
                  height="400"
                  width="300"
                  @mouseenter="toggle"
                  @mouseleave="toggle"
                >
                  <v-img
                    class="white--text align-start"
                    height="400px"
                    src="./assets/owners.jpg"
                  >
                    <v-card-title class="card-title-overlay f-martel">Our Story</v-card-title>
                    <v-expand-transition>
                      <v-card-text v-if="active"  class="card-title-overlay" style="height: 400px">Learn about...</v-card-text>
                    </v-expand-transition>
                  </v-img>
                </v-card>
              </v-slide-item>
            </v-slide-group>
          </v-fade-transition>
        </v-row>
      </v-container>

      <!-- <div class="hideme text--black">Fade In</div>
      <div style="height: 500px"/> -->
      
      <!-- fade in text + carousel -->
      <!-- add clickable cow, add any logic and screen effects to the landing page -->
    </v-main>
  </v-app>
</template>

<script>
//import Recipes from "./components/Recipes.vue";

export default {
  name: 'App',

  components: {
    //Recipes
  },

  data: () => ({
    scrolled: false,
    cardVisible: false,
    slide: null,
  }),

  created () {
    window.addEventListener('scroll', this.handleScroll);
  },

  methods: {
    handleScroll() {
      this.scrolled = window.scrollY != 0;
    },
    visibilityChanged(isVisible) {
      this.cardVisible = isVisible;
    }
  },

  destroyed () {
    window.removeEventListener('scroll', this.handleScroll);
  },
};
</script>
<style scoped>
.f-dancing {
  font-family: 'Dancing Script', cursive;
  /* font-size: 5rem; */
}
.f-martel {
  font-family: 'Martel', serif;
  /* font-size: 6rem; */
}
.stroke {
  -webkit-text-stroke: 2px white;
}
.h5-size {
  font-size: 5rem;
}
.h6-size {
  font-size: 8rem;
}
.card-title-overlay {
  background-color: rgba(0, 0, 0, 0.5);
}
#dairy {
  background: rgb(255, 254, 241)
}
</style>